...
3. 常用配置项介绍
   ...
   error_log  logs/error.log; # 错误日志所在路径, /usr/local/nginx
...
1. 静态网页存储目录
   ...
   - 自己创建新的目录:
     ...
     ```shell
     应该在 /usr/local/nginx/目录中创建自己的静态资源目录
...
1. 反向代理设置
   ...
   ## 用户端访问的时候必须选择不同的server_name访问才行。并且不能实现资源的平均调配。
   ```
...
2. 负载均衡设置
   ...
   ```nginx
       server {
           ...
           location /hello/ {  # 在根目录下的hello目录里面找
               # 反向代理服务器转发指令, http:// 固定的头
               proxy_pass http://linux.com;
           }
       	location /upload/ { # 在根目录下的upload目录里面找
               # 反向代理服务器转发指令, http:// 固定的头
               proxy_pass http://linux.com;
           }
...
* 是什么？
  ...
  开源框架：有一套API，有一套事件处理机制

* 能干什么？

  web服务器：http协议

  反向代理服务器：实现web服务器的负载均衡
...
* 怎么干活？
  ...
  ```
  # 处理静态网页
  1、制作出来网页，并且部署到对应的资源目录中
  2、根据客户端的请求，在服务端添加对应的location处理指令
  3、重新加载nginx配置文件
  客户端请求url：http://xxx.com/hello/login.html
  	- 去掉协议：http
  	- 去掉域名或者IP
  	- 去掉端口
  	- 去掉尾部文件名，即剩下：/hello/
  ...
  2、找到http --> server模块
  server {
  	listen: 80;				# 客户端访问反向代理服务器的端口
  	server_name: localhost;   # 域名，客户端访问反向代理服务器的域名
  	# 配置如何转发，根据客户端请求的url找到对应的转发指令
  	location / {
  		# 设置转发地址
  		proxy_path: http://kiko.com;
  	}
  	location /login {
  		# 设置转发地址
  		proxy_path: http://kiko.com;
  ...
  upstream kiko.com {
  	# web服务器的地址信息
  	server 192.168.47.131:80;
  	server 192.168.47.132:80;
  }
